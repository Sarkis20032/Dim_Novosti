...
def save_admin(message):
    ADMIN_IDS.add(message.text)
    bot.reply_to(message, f"Администратор {message.text} добавлен!")
...
def get_messages(message):
    ...
    bot.register_next_step_handler(message, show_client_messages)

def show_client_messages(message):
    user_id = message.text.strip()
    ...
    if messages:
        ...
        bot.reply_to(message, f"Сообщения клиента {user_id}:\n{text}")
    else:
        bot.reply_to(message, "У клиента нет сообщений.")

# Запоминаем входящие сообщения
